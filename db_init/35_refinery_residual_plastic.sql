...
SELECT 'Residual Plastic', b.id, 6.0, False
FROM buildings b
...
FROM recipes r, items i
WHERE r.name = 'Residual Plastic' AND i.name = 'Polymer Resin'
ON CONFLICT (recipe_id, item_id) DO NOTHING;

INSERT INTO recipe_ingredients (recipe_id, item_id, quantity)
SELECT r.id, i.id, 2.0
FROM recipes r, items i
WHERE r.name = 'Residual Plastic' AND i.name = 'Water'
ON CONFLICT (recipe_id, item_id) DO NOTHING;
