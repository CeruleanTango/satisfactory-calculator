...
INSERT INTO items (name, category) VALUES
  ('Ficsite Ingot', 'intermediate'),
  ('Ficsite Trigon', 'intermediate')
...
SELECT 'Ficsite Trigon', b.id, 6.0, False
FROM buildings b
...
SELECT r.id, i.id, 3.0, TRUE
FROM recipes r, items i
WHERE r.name = 'Ficsite Trigon'
...
FROM recipes r, items i
WHERE r.name = 'Ficsite Trigon' AND i.name = 'Ficsite Ingot'
ON CONFLICT (recipe_id, item_id) DO NOTHING;
